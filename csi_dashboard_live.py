...
def parse_line(line):
    """
    Expected shape: '<unix_timestamp> <v1>,<v2>,...,<v52>\n'
    Written directly by amma_40nights.py's patched receive loop -
    each line is already a real per-packet amplitude vector.
    """
    line = line.strip()
    if not line:
        return None
